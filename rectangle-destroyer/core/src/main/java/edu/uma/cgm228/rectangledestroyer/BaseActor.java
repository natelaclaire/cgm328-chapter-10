...
public class BaseActor extends Group
{
    ...
    public static ArrayList<BaseActor> getList(Stage stage, String className)
    {
        ...
        try
        {
            // Try to load class with fully qualified name first
            theClass = Class.forName(className);
        }
        catch (Exception error)
        {
            // If that fails, try prepending the package name
            try
            {
                theClass = Class.forName("edu.uma.cgm228.rectangledestroyer." + className);
            }
        ...
        // Only proceed if class was successfully loaded
        if (theClass != null)
        {
